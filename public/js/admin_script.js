$(function () {
  ...
  var getProductsInCategory = function (url) {
    ...
    $("div.product-list").append(
      '<img style="position: absolute; left: 0; top: 0; z-index: 100000;" src="' +
        baseUrl +
        '/images/loading.gif" />'
    );
  ...
  var getPropertyTypes = function () {
    $.get(baseUrl + "/admin/products/property-types/")
      .done(function (data) {
  ...
  var getPropertyValues = function (id) {
    $.get(baseUrl + "/admin/products/property/" + id + "/values/")
      .done(function (data) {
  ...
  var updateProductProperties = function (product_id) {
    $.get(baseUrl + "/admin/product/" + product_id + "/properties/")
      .done(function (data) {
  ...
  $(document).on("click", "#removeProperty", function () {
    $("div[id^=product-property]").append(
      '<img style="position: absolute; left: 0; top: 0; z-index: 100000;" src="' +
        baseUrl +
        '/images/loading.gif" />'
    );
  ...
  // add product
  $(document).on("click", "#add-property-modal-button-add", function () {
    var value = $("#propertyTypeValue").val();
    var product_id = $("input#modal-product-id").val();
    var input_type = $("#propertyTypeValue").data("input-type");
    var property_id = $("#propertyTypeValue").data("property-id");
  ...
  $(document).on("show.bs.modal", "#propertyModal", function (e) {
    ...
    $(e.delegateTarget)
      .find("div[id^=product-property]")
      .each(function () {
        var removeOptionId = $(this).data("prop-id");
        $(
          'select#propertyType option[value="' + removeOptionId + '"]'
        ).remove();
      });
    $(".selectpicker").selectpicker("refresh");
  ...
  $(document).on("click", "#save-new-property-modal-button", function (e) {
    $.post(baseUrl + "/admin/properties", {
      ...
      property_type: $(
        "input[type=radio][name=new-property-type]:checked"
      ).val(),
  ...
  $(document).on(
    "click",
    "input[type=radio][name=new-property-type]",
    function () {
  ...
  var fillChangeStatusLinkText = function () {
    $(".shipping-method-change-status").each(function () {
      var status = $(this).data("status") ? "деактивувати" : "активувати";
      $(this).text(status);
    });
  };
